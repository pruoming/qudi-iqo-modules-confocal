# -*- coding: utf-8 -*-
"""
JSON exchange IO for qudi pulse objects — phase 0 of the qudi-independent pulse editor.

Contract: Qudi_AI/tools/pulse_editor/pulse_sequence_format.md, format_version 1 (consultant
T1+T2, owner-approved 2026-08-10). This module is deliberately THIN: it moves data between
JSON and the dict representation that the stock pulse objects already define
(get_dict_representation / *_from_dict) and hands persistence to the stock
...
Import hard rules (format doc §7): reject format_version > 1; validate sampling-function
names against the INSTALLED SamplingFunctions set before construction (unknown name = clear
error naming the offender and the legal set — never a guess); element dicts must carry
exactly the five contract keys; block_list arrays are normalized back to (str, int) tuples;
a referenced block/ensemble must be in the file or already saved on the importing side.

Export hard rules: allow_nan=False (non-finite numbers fail loudly); numpy SCALARS are
coerced to plain Python numbers; numpy ARRAYS are rejected in v1 with a clear message
(empty the transient dicts or convert to lists first).
...
class PulseJsonError(ValueError):
    """Raised on any violation of the pulse-sequence JSON exchange contract (v1)."""
    pass
...
def _json_default(obj):
    ...
    if isinstance(obj, np.ndarray):
        raise PulseJsonError(
            'numpy arrays are not representable in pulse-JSON v1 (found one while exporting). '
    ...
    raise PulseJsonError('Object of type {0} is not representable in pulse-JSON v1.'
                         ''.format(type(obj).__name__))


def export_to_json(file_path=None, blocks=None, ensembles=None, sequences=None):
    """Serialize pulse objects to a format_version-1 JSON container.
...
def _validate_element_dict(element_dict, where):
    ...
    if keys != _ELEMENT_KEYS:
        raise PulseJsonError(
            '{0}: element keys must be exactly {1}; got {2}. (Contract §3 — the element dict '
            'feeds the PulseBlockElement constructor verbatim.)'.format(
                where, sorted(_ELEMENT_KEYS), sorted(keys)))
...
def import_from_json(source, sequence_generator_logic=None, save=True):
    """Import a format_version-1 JSON container into pulse objects.
    ...
    for blk_dict in container.get('blocks', []):
        if not isinstance(blk_dict, dict) or set(blk_dict) != {'name', 'element_list'}:
            raise PulseJsonError('Block entries must carry exactly "name" and '
                                 '"element_list" (contract §4).')
        ...
        # deepcopy: the stock *_from_dict mutate their input in place
        block = PulseBlock.block_from_dict(copy.deepcopy(blk_dict))
